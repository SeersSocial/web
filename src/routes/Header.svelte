...
<div class="relative px-8 bg-black text-white">
	...
	>
		...
		<NavHamburger style="background:black" on:click={toggle} class="" btnClass="text-white focus:outline-none whitespace-normal m-0.5 rounded-lg focus:ring-2 p-1.5 focus:ring-gray-400  bg-black hover:bg-black  ml-3 md:hidden" />
		<NavUl {hidden} style="background:black" activeClass="bg-black" divClass="bg-black" nonActiveClass="bg-black" ulClass="bg-black dark:bg-black text-white dark:text-white rounded-lg border border-black dark:border-black flex flex-col p-4 mt-4 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium">
